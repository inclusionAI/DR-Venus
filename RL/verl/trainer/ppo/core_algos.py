...
@register_adv_est(AdvantageEstimator.MTGRPO)
def compute_turn_grpo_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    """
    Compute the advantage with discounted turn-level returns for Multi-Turn GRPO.

    Per-token returns R_t are computed recursively from the end of the sequence:
        R_t = r_t + gamma * R_{t+1}

    Args:
        token_level_rewards: `(torch.Tensor)`
            Per-token immediate rewards of shape (bs, response_length).
        response_mask: `(torch.Tensor)`
            Response-sequence mask of shape (bs, response_length).
        index: `(np.ndarray)`
            Prompt-index array used to group samples.
        epsilon: `(float)`
            Small constant to avoid divide-by-zero during normalization.
        norm_adv_by_std_in_grpo: (bool)
            Whether to scale the GRPO advantage by its standard deviation.
            True  -> advantage is scaled by std (original GRPO).
            False -> no scaling (Dr.GRPO-style, https://arxiv.org/abs/2503.20783).

    Returns:
        advantages: `(torch.Tensor)` of shape (bs, response_length).
            Per-token advantages after baseline subtraction (and optional std scaling).
        returns: `(torch.Tensor)` of shape (bs, response_length).
            Discounted cumulative returns G_t per token.
    """
    ...
    # Step 1: compute per-token discounted returns by iterating backward over the sequence.
    ...
    for t in reversed(range(seq_len)):
        ...
        gamma_turn_if_reward_nonzero = torch.where(next_return < 0.0, 0.0, 0.1)
        gamma_turn = torch.where(current_reward == 0.0, 1.0, gamma_turn_if_reward_nonzero)
        current_return = current_reward + gamma_turn * next_return
    ...
    with torch.no_grad():
        # Step 2: group all return sequences by their prompt index.
        for i in range(bsz):
            id2returns[index[i]].append(discounted_returns[i])
            id2mask[index[i]].append(response_mask.float()[i])

        # For each group, compute a scalar baseline (mean/std) aggregated over
        # per-sample sequence means within the group.
        for idx, group_returns_list in id2returns.items():
            group_returns_tensor = torch.stack(group_returns_list, dim=0)
            group_mask_tensor = torch.stack(id2mask[idx], dim=0)
            id2mean[idx] = group_returns_tensor.mean()
            id2std[idx] = group_returns_tensor.std()
            
            row_count = group_mask_tensor.sum(dim=1)    
            row_count = torch.clamp(row_count, min=1.0)
            row_mean = (group_returns_tensor * group_mask_tensor).sum(dim=1) / row_count
            mean = row_mean.mean()
            std = row_mean.std()
    ...
    # Step 3: subtract the group baseline from per-token returns to obtain advantages.
    ...
    # Step 4: mask out padding positions.
