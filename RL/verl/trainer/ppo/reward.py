...
def compute_reward(data: DataProto, reward_fn: AbstractRewardManager, val_type,
                   info_gain_rewards=None) -> tuple[torch.Tensor, dict[str, Any]]:
    """
    Compute reward for a batch of data.
    Args:
        data: DataProto object containing the input data.
        reward_fn: Reward function to compute the reward.
        val_type: Validation type string.
        info_gain_rewards: Optional list of per-sample info-gain reward lists (IGPO feature).
    Returns:
        Tuple of reward tensor and extra info dictionary.
    """
    kwargs = dict(return_dict=True, val_type=val_type)
    if info_gain_rewards is not None:
        kwargs["info_gain_rewards"] = info_gain_rewards
    reward_result = reward_fn(data, **kwargs)
    reward_tensor = reward_result["reward_tensor"]
    reward_extra_infos_dict = reward_result.get("reward_extra_info", {})

    if info_gain_rewards is not None:
        reward_extra_infos_dict["_ig_status"] = {
            "requested": True,
            "applied": True,
        }
...
def compute_reward_async(data: DataProto, config, tokenizer, global_steps, val_type):
    ...
    if "info_gain_rewards" in data.non_tensor_batch:
        _ig_rw = [list(x) if x is not None else [] for x in data.non_tensor_batch["info_gain_rewards"]]
    reward_tensor, reward_extra_infos_dict = compute_reward(
        data, reward_fn, val_type=val_type, info_gain_rewards=_ig_rw)
