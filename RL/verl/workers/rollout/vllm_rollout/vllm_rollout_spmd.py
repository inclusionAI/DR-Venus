...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            print(f'self.sampling_params-----{self.sampling_params}---，kwargs {kwargs}')
            # print(f'vllm_inputs {max([len(x['prompt_token_ids']) for x in vllm_inputs])}')
            ...
            try:
                ...
                _has = False
                for _o in outputs:
                    _total_prompt += len(_o.prompt_token_ids)
                    _nc = getattr(_o, 'num_cached_tokens', None)
                    if _nc is not None:
                        _has = True
                        _total_cached += _nc
                if _has and _n_seqs > 0:
                    print(f"[APC] new_per_seq={(_total_prompt-_total_cached)/_n_seqs:.0f} "
                          f"batch={_n_seqs}", flush=True)
            except Exception as _e:
                print(f"[APC] error: {_e}", flush=True)
