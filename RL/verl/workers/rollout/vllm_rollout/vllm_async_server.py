...
class AsyncvLLMServer(AsyncServerBase):
    ...
    async def init_engine(self):
        ...
        if not config.enforce_eager and cudagraph_capture_sizes:
            if isinstance(cudagraph_capture_sizes, ListConfig):
                if CompilationConfig is not None and CompilationLevel is not None:
                    ...
                    logger.warning("CompilationConfig/CompilationLevel not available in this vLLM version, skipping cudagraph config")
            else:
    ...
    async def generate(
        ...
        _lp = sampling_params.pop("logprobs", None)
        if isinstance(_lp, bool):
            sampling_params["logprobs"] = 0 if _lp else None
        else:
            sampling_params["logprobs"] = _lp
        ...
        # Get final response, measuring TTFT (prefill) and decode time
        t_start = _time.time()
        t_first_token = None
        ...
        prefill_time = (t_first_token - t_start) if t_first_token is not None else (t_end - t_start)
        decode_time = (t_end - t_first_token) if t_first_token is not None else 0.0

        token_ids = final_res.outputs[0].token_ids
        log_probs = None
        if sampling_params.logprobs is not None:
            log_probs = [logprobs[token_ids[i]].logprob for i, logprobs in enumerate(final_res.outputs[0].logprobs)]
        return TokenOutput(token_ids=token_ids, log_probs=log_probs, prefill_time=prefill_time, decode_time=decode_time)
    ...
    async def compute_prompt_logprobs(
        ...
        """Return per-position prompt log-probs via vLLM ``prompt_logprobs``.

        Uses ``max_tokens=1`` (generates one throwaway token) so the engine
        runs a full prefill on *prompt_ids* and returns the log-probability of
        each prompt token given its preceding context.  With prefix-caching
        enabled, previously-seen prefixes are served from cache and only the
        new suffix actually goes through the model.
        """
        ...
        generator = self.engine.generate(
            ...
            request_id=f"{request_id}_plp",
        )
        final_res = None
        async for output in generator:
            final_res = output
        assert final_res is not None
        return final_res.prompt_logprobs
