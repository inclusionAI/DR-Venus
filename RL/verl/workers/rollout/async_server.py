...
class TokenOutput(BaseModel):
    ...
    """Time in seconds from generate() call to first token (≈ prefill + scheduling)"""
    decode_time: float = 0.0
    """Time in seconds from first token to last token (≈ decode)"""
...
class AsyncServerBase(ABC):
    ...
    async def compute_prompt_logprobs(
        ...
        """Compute log-probabilities for each token in *prompt_ids*.

        Returns a list aligned with *prompt_ids* where each element is either
        ``None`` (position not scored, e.g. first token or cache-hit) or a
        ``dict`` mapping ``token_id → Logprob``.  Subclasses may override;
        the default raises ``NotImplementedError``.
        """
        raise NotImplementedError(
            f"{type(self).__name__} does not support compute_prompt_logprobs"
        )
