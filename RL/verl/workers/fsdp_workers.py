...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if use_meta_tensor:
            _fsdp_param_init_fn = init_fn
            _fsdp_sync_module_states = True
        else:
            _fsdp_param_init_fn = None
            _fsdp_sync_module_states = False
        ...
        if fsdp_strategy == "fsdp":
            actor_module_fsdp = FSDP(
                ...
                sync_module_states=_fsdp_sync_module_states,
    ...
    @DistProfiler.annotate(color="red", role="convert_to_rollout")
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="rollout"))
    @DistProfiler.annotate(color="red", role="convert_to_trainer")
    def convert_to_trainer(self):
        if self._is_actor:
            loop = asyncio.get_event_loop()
            loop.run_until_complete(self.trainer_mode())
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="rollout"))
    @DistProfiler.annotate(color="red", role="rollout_generate_continue")
    def generate_sequences_continue(self, prompts: DataProto):
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    def compute_ig_with_kv_cache(self, data: DataProto):
        """Compute info-gain log probs via KV cache reuse (runs entirely on worker).

        Data is DP-split across actor ranks. Each rank processes its subset of
        samples. The original_sample_idx tensor maps local->global indices for
        turn boundary lookup.
        """
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        with self.ulysses_sharding_manager:
            result = self.actor.compute_ig_with_kv_cache(data)
