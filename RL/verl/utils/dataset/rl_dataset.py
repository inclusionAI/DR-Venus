...
class AsycRMDataset(RLHFDataset):
    ...
    def _init_streaming_state(self):
        from verl.utils.fs import copy_to_local
        # Streaming only consumes the first data file; remote backends are not yet supported.
    ...
    def _load_streaming_data(self):
        ...
        if file_timestamp <= self.last_file_timestamp:
            return  # file not updated since last load
    ...
    def __getitem__(self, item):
        waittime = 0
        # Skip samples whose deadline (ddl) has already passed.
        ...
        while item + self.offset >= len(self.dataset):
            ...
            # Once this loop is reached, new data must arrive before training can proceed.
    ...
    def __len__(self):
        return 1000000  # effectively infinite for a streaming dataset

    def _update_reward(self):
        def align_dp(batch):
            # Find the longest prompt and response within the batch.
        ...
        # Look up ground-truth labels by prompt and then compute rewards.
        ...
        while (True):
            ...
            if len(rollout_datas) > 0:
                ...
                for prompt in rollout_datas.non_tensor_batch['raw_prompt'].tolist():
                    if prompt not in withrm_prompt:
                        # A timestamp-based sanity check could be added here.
            ...
            # Compute rewards for the selected rollout indices.
