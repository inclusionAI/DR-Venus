...
from tools_server.http_client import MessageClient as HttpClient
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.client = HttpClient(isconsumer=True)
    ...
    def _validate(self):
        ...
        metric_dict.update({'val/turns/mean': np.mean([int(x.split('||')[0].split('_')[2])
                           for x in test_batch.non_tensor_batch['agent_grpo_idx_deepthink'] if '_answer' in x])})
        metric_dict.update({'val/turns/median': np.median([int(x.split('||')[0].split('_')[2])
                           for x in test_batch.non_tensor_batch['agent_grpo_idx_deepthink'] if '_answer' in x])})
        reward_extra_infos_dict['agent_grpo_idx_deepthink'] = test_batch.non_tensor_batch['agent_grpo_idx_deepthink'].tolist()
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # 根据 agent_grpo_idx 对每个uid进行不同次数的repeat
                    ids = np.array(ids)
                    ...
                    # batch总样本数必须是world_size的倍数，如果不是，随机增加几个
                    ...
                    if rollout_data_dir:
                        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
                            ...
                            # print('token_level_scores',batch.batch["token_level_scores"].sum(-1).cpu().tolist()[:10])
                            nonzero_mask = reward_tensor != 0                          # [B, T]
                            rev_first_idx = nonzero_mask.flip(dims=[1]).float().argmax(dim=1)
                            last_nonzero_indices = reward_tensor.size(1) - 1 - rev_first_idx
                            ...
                            metrics.update({'train/turns/mean':np.mean([int(x.split('||')[0].split('_')[2]) for x in gen_batch_output.non_tensor_batch['agent_grpo_idx_deepthink'] if '_answer' in x])})
                            metrics.update({'train/turns/median':np.median([int(x.split('||')[0].split('_')[2]) for x in gen_batch_output.non_tensor_batch['agent_grpo_idx_deepthink'] if '_answer' in x])})
                            metrics.update({'train/acc':np.mean([1 if x >= 1 else 0 for x in scores])})
